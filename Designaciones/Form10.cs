...
namespace Designaciones
{
    public partial class Form10 : Form
    {
        ...
        string archivo = Directory.GetCurrentDirectory() + "\\ReporteAsignacionesPorArtefacto.html";

        private void button1_Click(object sender, EventArgs e)
        {
            //Generar
            StreamWriter arch = new StreamWriter(archivo);
            arch.WriteLine("<html>REPORTE DE ASIGNACIONES POR ARTEFACTO<br><br>");
            arch.WriteLine("<table border=1 cellspacing=0>");
            arch.WriteLine("<tr><td>Artefacto</td><td>Asignaciones Totales</td></tr> ");
            string connectionString =
            "datasource=localhost;port=3306;username=root;password=;database=designaciones;";
            string query = "select artefacto, total from artefactos join(select id_artefacto, count(id_asignacion)total from asignaciones group by id_artefacto) t on artefactos.id_artefacto = t.id_artefacto;";
